...
the related rows in the form, and a related-label column in the table. **Every related-label surface — the
select (current value + remote search), the list column (display, filter, sort), the list-filter dropdown,
the API resource, and the show/trash views — resolves the related model's display column the same way**: an
explicit `displayColumn()` method on the related model, else the first of `name` → `title` → `label` present
on its table, else the route key (so a `title`-based model like a Course labels/filters/sorts by `title`, and
