...
it('is filesystem-only — it never creates permission rows (that is admin-core:sync-permissions)', function () {
    // The architecture split: admin-core:make writes files + config, and NEVER mutates the runtime database.
    // Even with the permissions table present and permissions enabled, no permission row is created — the
    // routes it generates gate on list-gizmo etc., and admin-core:sync-permissions creates + grants them.
    // (Group-filing under a hybrid-key group_permissions table is covered in SyncPermissionsTest.)
    ...
    expect(\Illuminate\Support\Facades\DB::table('permissions')->count())->toBe(0);
});
...
it('re-uses the existing portal scope when re-running make without --portal', function () {
    ...
    // The documented add-a-channel workflow — but forgetting to restate --portal. Without the scope
