...
class AdminCoreMakeCommand extends Command
{
    protected $signature = 'admin-core:make {name? : The resource name, e.g. Product}
                            ...
                            {--read-only : A read-only resource — list, show, export + filters/totals, but NO create/edit/delete/import (a report). Only the list route is permission-gated.}
                            {--singleton : A one-record screen — an edit form (index) + save (update), NO list/create/delete (Settings, a profile). Only the edit route is permission-gated.}
    ...
    protected $description = 'Scaffold a full admin-core CRUD resource (model, service, controller, requests, routes, views, tests) — filesystem only. Run admin-core:sync-permissions to create + grant its permissions.';
    ...
    public function handle(): int
    {
        ...
        // NOTE: admin-core:make is filesystem-only — it does NOT create permissions or grant roles. The
        // routes it generates already gate on `permission:{action}-{resource}`; run the (idempotent,
        // database-availability-aware) `admin-core:sync-permissions` command to create + grant them. This
        // keeps code generation working offline / in CI / against an unreachable database.
        if ($web) {
        ...
        $this->line("  Run <info>php artisan migrate</info>"
            . ($web ? ", then visit /admin/{$snakePlural}." : ' — API routes load from routes/Api/Modules.'));
        $this->line("  Run <info>php artisan admin-core:sync-permissions</info> to create + grant the {$kebab} permissions "
            . '(make is filesystem-only — it does not touch the database).');
...
PHP);
     ...
     * Delegates to the shared {@see PermissionNaming} so the stubs this command bakes and the
     * `admin-core:sync-permissions` engine can never derive a different name.
     */
    private function permissionName(string $action, string $kebab): string
    {
        return PermissionNaming::name($action, $kebab);
    }
